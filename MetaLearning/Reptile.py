...
try:
    for i in range(meta_epoch):
        ...
        meta_loss = 0
        site_dir = random.sample(indexPool, 6)
        meta_optimizer.zero_grad()
        for site_index in site_dir:
            taskname = dir[site_index]
        ...
        for main_param, clone_param in zip(model.parameters(), model_copy.parameters()):
            if main_param.grad is None:
                main_param.grad = Variable(torch.zeros(main_param.size())).cuda()
            main_param.grad = main_param.grad + (main_param.data - clone_param.data)
        optimizer.step()
        optimizer.zero_grad()
        scheduler.step()
        torch.cuda.empty_cache()
    ...
    torch.save({
                ...
                }, 'C:/telun/FCN/Evaluation_exp2/Reptile_FCN/model/Reptile_init_v1.pth')
    ...
    txtpath = 'C:/telun/FCN/Evaluation_exp2/Reptile_FCN/record/MetaTime.txt'
    f = open(txtpath, 'w')
    f.write(str(Time))
    f.close()
